...
/*
	...
	void TBusComponent::placeInitialPoint(const sf::Vector2f& PointPos)
	{
		...
		VertexArray.append({ { PointPos.x - (mThickness / 2.f), PointPos.y - (mThickness / 2.f) }, mBusColor });
		VertexArray.append({ { PointPos.x - (mThickness / 2.f), PointPos.y + (mThickness / 2.f) }, mBusColor });
		VertexArray.append({ { PointPos.x + (mThickness / 2.f), PointPos.y + (mThickness / 2.f) }, mBusColor });
		VertexArray.append({ { PointPos.x + (mThickness / 2.f), PointPos.y - (mThickness / 2.f) }, mBusColor });
	...
	void TBusComponent::lineToQuad(const sf::Vector2f& LineBegin, const sf::Vector2f& LineEnd)
	{
		...
		if (checkOrentation(LineBegin, LineEnd))
		{
			VertexArray.append({ { LineBegin.x, LineBegin.y - (mThickness / 2) }, mBusColor });
			VertexArray.append({ { LineBegin.x, LineBegin.y + (mThickness / 2) }, mBusColor });
			VertexArray.append({ { LineEnd.x, LineEnd.y + (mThickness / 2) }, mBusColor });
			VertexArray.append({ { LineEnd.x, LineEnd.y - (mThickness / 2) }, mBusColor });
		}
		// In case of vertical alignment
		else
		{
			VertexArray.append({ { LineBegin.x - (mThickness / 2), LineBegin.y }, mBusColor });
			VertexArray.append({ { LineBegin.x + (mThickness / 2), LineBegin.y }, mBusColor });
			VertexArray.append({ { LineEnd.x + (mThickness / 2), LineEnd.y }, mBusColor });
			VertexArray.append({ { LineEnd.x - (mThickness / 2), LineEnd.y }, mBusColor });
		}
